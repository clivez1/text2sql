...
> 注：本文档原始 Phase 命名与 devfile 阶段目录的映射关系：
> Phase A = stage0, Phase B = stage1, Phase C = stage2, Phase D = stage3, Phase E = stage4, Phase F = stage5

### stage0（Phase A）：文档与边界冻结
...
### stage3（Phase D）：多轮 Copilot 和结果表达
...
- 按 stage1/01-repository-physical-reorg/ 冻结逐目录物理整理顺序
- 按 stage1/02-code-refactor/ 冻结逐层代码迁移批次和 cutover 规则
- 以 stage1/02-code-refactor/07-refactor-kickoff.md 作为正式进入代码重构阶段的统一入口
...
- references/architecture/01-target-architecture.md：目标架构边界
- stage1/02-code-refactor/07-refactor-kickoff.md：当前直接重构入口
- references/themes/01-llm-and-retrieval.md：LLM 优先和大库检索
- references/themes/02-copilot-visualization-and-actions.md：多轮、可视化与治理动作
- references/themes/03-evaluation-and-release.md：评测与门禁
- stage1/01-repository-physical-reorg/：仓库逐目录物理整理方案
- stage1/02-code-refactor/：逐层代码迁移和 cutover 方案