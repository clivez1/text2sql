...
# 向后兼容：RULES 曾直接从 generator.py 导出
RULES = RuleStore.get_instance().rules
...
def try_template_sql(question: str) -> tuple[str, str] | None:
    ...
    if (
        metric == "SUM(oi.quantity * oi.unit_price)"
        and "产品" in question
        and "前5" in question
    ):
    ...
    if (
        "区域" in question
        and "销售额" in question
        and any(region in question for region in ["华东", "华南", "华北"])
    ):
        region = next(
            region for region in ["华东", "华南", "华北"] if region in question
        )
        return (
