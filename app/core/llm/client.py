...
from app.core.llm.adapters import BailianCodePlanAdapter, LLMAdapter, OpenAICompatibleAdapter
...
def get_llm_adapter() -> LLMAdapter:
    ...
    if config.provider == "bailian_code_plan":
        return BailianCodePlanAdapter(config=config)
    if config.provider == "openai_compatible":
        return OpenAICompatibleAdapter(config=config)
    if config.provider == "astron":
        return OpenAICompatibleAdapter(config=config)
